...
class GameClient {
   ...
   // Убедимся, что метод startNewRound в классе GameClient правильно работает
   startNewRound() {
       if (this.roomId) {
           console.log("Запрос на начало нового раунда для комнаты:", this.roomId);
    ...
    // Новый метод для завершения текущего раунда
    finishRound() {
        if (this.roomId) {
            console.log("Запрос на завершение текущего раунда для комнаты:", this.roomId);
            this.socket.emit('finish-round', this.roomId);
        } else {
            console.error('Невозможно завершить раунд без активной комнаты');
        }
    }
}
